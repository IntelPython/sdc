...
def agg_parallel_local_iter(key_arr, data_in, shuffle_meta, data_shuffle_meta, __update_redvars):
    ...
    for i in range(len(key_arr)):
        ...
        #update_all_redvars(redvar_arrs, data_in, w_ind, i)
...
def gen_top_level_agg_func(key_typ, return_key, red_var_typs, out_typs,
    ...
    if parallel:
        ...
        in_args = ", ".join(out_names + recv_names)
        if len(out_names) != 0:
            in_args = ", " + in_args
...
# def gen_agg_func_parallel_local(agg_func_struct, key_typ, in_typs, out_typs, typingctx,
...
#     iter_func = gen_agg_iter_func_parallel_local(
...
# def gen_agg_iter_func_parallel_local(key_typ, red_var_typs, num_ins, num_outs, num_red_vars,
...
#     func_text = "def f(key_arr, data_in, shuffle_meta, data_shuffle_meta):\n"
...
#             i, i, typ)
#         func_text += "    data_shuffle_meta[{}] = redvar_{}_arr\n".format(i, i)


#     # key is returned in parallel local agg phase (TODO: avoid if key is output already)
#     if parallel_local:
#         func_text += "    out_key = shuffle_meta.send_buff\n"
...
#         func_text += "        w_ind = shuffle_meta.send_disp[node_id] + shuffle_meta.tmp_offset[node_id]\n"
#         func_text += "        shuffle_meta.tmp_offset[node_id] += 1\n"
#     else:
