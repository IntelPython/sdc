...
so = "/localdisk/work/fschlimb/miniconda3/envs/HPAT/lib/python3.6/site-packages/daal4py-0.2018.20180412-py3.6-linux-x86_64.egg/_daal4py.cpython-36m-x86_64-linux-gnu.so"
...
class algo_factory(object):
    ...
    """
    def __init__(self, algo, c_name, param_types, input_types, result_attrs):
        """
        factory to build the numba types and lowering stuff.
          ...
          - input_types: list of numba types for the algo input arguments (to compute)
          - result_attrs: list of pairs (name, numba-type) represnting result attributes
        """
        self.algo = algo
        self.c_name = c_name
    ...
    def mk_ctor(self):
        ...
        def ctor_impl(context, builder, sig, args):
            ...
            fnty = lir.FunctionType(lir.IntType(8).as_pointer(), # ctors always just return an opaque pointer
                                    [get_lir_type(context, x) for x in self.param_types])
    ...
    def mk_compute(self):
        ...
        @lower_builtin(compute_name, self.nbtype_algo, *self.input_types)
...
daal4py_algos = [
    algo_factory(daal4py.kmeans_init,
                 'kmeans_init',
                 [types.intp, string_type, string_type, types.intp, types.float64, types.intp, types.boolean],
                 [dtable_type],
                 [('centroids', dtable_type)],
    ),
    algo_factory(daal4py.kmeans,
                 'kmeans',
                 [types.intp, types.intp, string_type, string_type, types.float64, types.float64, string_type, types.boolean],
...
def full_kmeans(a, nClusters, fptype, initmethod, seed, oversamplingFactor, nRounds, maxIterations, method, accuracyThreshold, gamma, assignFlag, distributed):
    kmi = daal4py.kmeans_init(nClusters, fptype, initmethod, seed, oversamplingFactor, nRounds, distributed)
    kmir = kmi.compute(a)
    km = daal4py.kmeans(nClusters, maxIterations, fptype, method, accuracyThreshold, gamma, assignFlag, distributed)
...
a = loadtxt(infile, delimiter=',')

print(full_kmeans(a, 2, "double", "defaultDense", 111, 2.2, 33, 300, "lloydDense", 0.00001, 0.1, "True", False))
print(fkm(a, 2, "double", "defaultDense", 111, 2.2, 33, 300, "lloydDense", 0.00001, 0.1, "True", False))
